...
Strings SplitStringOnDelimiter(std::string const& originalString, char delimiterToSplitOn)
{
	...
	while (true)
	{
		size_t delimiterPos = originalString.find(delimiterToSplitOn, start);

		if (delimiterPos == std::string::npos)
		{
			result.push_back(std::string(originalString, start));
			break;
		}
		else
		{
			result.push_back(std::string(originalString, start, delimiterPos - start));
