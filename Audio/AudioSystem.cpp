...
static FMOD_VECTOR ToFmodVector(Vec3 const& gameVec)
{
	FMOD_VECTOR result;
	result.x = -gameVec.y;
	result.y = gameVec.z;
	...
	Vec3 const& listenerUp)
{
	...
	FMOD_VECTOR pos = ToFmodVector(listenerPosition);
	FMOD_VECTOR vel = ToFmodVector(Vec3::ZERO);
	FMOD_VECTOR fwd = ToFmodVector(listenerForward.GetNormalized());
	FMOD_VECTOR up = ToFmodVector(listenerUp.GetNormalized());
...
SoundPlaybackID AudioSystem::StartSoundAt(
	...
	bool isPaused)
{
	SoundPlaybackID playbackID = StartSound(
		soundID,
		isLooped,
		volume,
		balance,
	...
	FMOD_VECTOR pos = ToFmodVector(soundPosition);
	FMOD_VECTOR vel = ToFmodVector(Vec3::ZERO);
...
void AudioSystem::SetSoundPosition(SoundPlaybackID playbackID, Vec3 const& soundPosition)
{
	FMOD::Channel* channel = (FMOD::Channel*)playbackID;
	if (channel == nullptr)
	{
...
bool AudioSystem::IsPlaying(SoundPlaybackID playbackID) const
{
	FMOD::Channel* channel = (FMOD::Channel*)playbackID;
	if (channel == nullptr)
	{
